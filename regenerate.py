...
README_MARK_START = "<!-- LIBRARY_START -->"
README_MARK_END = "<!-- LIBRARY_END -->"
...
def render_readme_library(manifest: dict) -> str:
    lines: list[str] = []
    for c in manifest["categories"]:
        lines.append(f"### {c['name']}")
        lines.append("")
        for s in c["subcategories"]:
            if s["name"] and s["name"] != "_default":
                lines.append(f"**{s['name']}**")
                lines.append("")
            for it in s["items"]:
                desc = f" — {it['description']}" if it.get("description") else ""
                lines.append(f"- [{it['title']}]({it['file']}){desc}")
            lines.append("")
    return "\n".join(lines).rstrip() + "\n"


def rewrite_readme(manifest: dict) -> None:
    if not README.exists():
        return
    text = README.read_text(encoding="utf-8")
    start = text.find(README_MARK_START)
    end = text.find(README_MARK_END, start + len(README_MARK_START) if start != -1 else 0)
    if start == -1 or end == -1:
        print(
            f"  ! skipping README: markers missing. Add these two lines where the library should go:\n"
            f"      {README_MARK_START}\n      {README_MARK_END}",
            file=sys.stderr,
        )
        return
    body = render_readme_library(manifest)
    new_block = (
        f"{README_MARK_START}\n"
        f"<!-- This section is auto-generated by regenerate.py — do not edit by hand. -->\n\n"
        f"{body}\n"
    )
    new_text = text[:start] + new_block + text[end:]
